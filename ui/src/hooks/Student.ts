...
const getStudentByKey = (id: number) =>
  axios
    .get<ODataResponse<StudentType>>(
      `${import.meta.env.VITE_API_URL}/odata/Student?$filter=Id eq ${id}&$expand=Courses`
    )
...
export const useStudentByKey = (id: number) =>
  useQuery({
    queryKey: ["Student", id],
    queryFn: () => getStudentByKey(id),
  });
